...
@section('title', 'Import CSV Strategi 8')
@section('page-title', 'Import Data Strategi 8 - Wingback Penguatan Produk & Fungsi RM dari CSV')
...
<div class="import-container">
    ...
    <form action="{{ route('strategi8.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('strategi8.index') }}" class="btn btn-secondary">
                ↩️ Kembali
            </a>
        </div>
    </form>
    ...
    @php
        $totalStrategi8 = \App\Models\Strategi8::count();
    @endphp
    ...
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">
            Menghapus semua data strategi 8 akan menghapus <strong>{{ number_format($totalStrategi8, 0, ',', '.') }} data</strong> secara permanen dan tidak dapat dikembalikan.
        </p>
        <form action="{{ route('strategi8.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA {{ number_format($totalStrategi8, 0, ",", ".") }} data strategi 8!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">
                🗑️ Hapus Semua Data Strategi 8
            </button>
        </form>
    </div>
    @endif
