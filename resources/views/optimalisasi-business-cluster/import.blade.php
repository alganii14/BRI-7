...
@section('title', 'Import CSV Optimalisasi Business Cluster')
@section('page-title', 'Import Data Optimalisasi Business Cluster dari CSV')
...
<div class="import-container">
    ...
    <div class="info-box">
        ...
        <ul>
            ...
            <li>Kolom CSV: No, Branch Office, Branch Code, Unit Kerja, TAG ZONA UNGGULAN, Nomor Rekening Tab, Nama Pusat Bisnis, Nama Tenaga Pemasar</li>
        </ul>
    </div>

    <form action="{{ route('optimalisasi-business-cluster.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('optimalisasi-business-cluster.index') }}" class="btn btn-secondary">
                ↩️ Kembali
            </a>
        </div>
    </form>
    ...
    @php
        $totalOptimalisasiBusinessCluster = \App\Models\OptimalisasiBusinessCluster::count();
    @endphp

    @if($totalOptimalisasiBusinessCluster > 0)
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">
            Menghapus semua data optimalisasi business cluster akan menghapus <strong>{{ number_format($totalOptimalisasiBusinessCluster, 0, ',', '.') }} data</strong> secara permanen dan tidak dapat dikembalikan.
        </p>
        <form action="{{ route('optimalisasi-business-cluster.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA {{ number_format($totalOptimalisasiBusinessCluster, 0, ",", ".") }} data optimalisasi business cluster!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">
                🗑️ Hapus Semua Data Optimalisasi Business Cluster
            </button>
        </form>
    </div>
    @endif
