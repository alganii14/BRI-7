...
@section('title', 'Import CSV Perusahaan Anak')
@section('page-title', 'Import Data List Perusahaan Anak dari CSV')
...
<div class="import-container">
    ...
    <form action="{{ route('perusahaan-anak.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('perusahaan-anak.index') }}" class="btn btn-secondary">
                ↩️ Kembali
            </a>
        </div>
    </form>

    @if($totalPerusahaanAnak > 0)
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">
            Menghapus semua data perusahaan anak akan menghapus <strong>{{ number_format($totalPerusahaanAnak, 0, ',', '.') }} data</strong> secara permanen dan tidak dapat dikembalikan.
        </p>
        <form action="{{ route('perusahaan-anak.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA {{ number_format($totalPerusahaanAnak, 0, ",", ".") }} data perusahaan anak!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">
                🗑️ Hapus Semua Data Perusahaan Anak
            </button>
        </form>
    </div>
    @endif
