...
@section('title', 'Import CSV Brilink')
@section('page-title', 'Import Data Brilink dari CSV')
...
<div class="import-container">
    ...
    <div class="info-box">
        ...
        <ul>
            ...
            <li>Kolom CSV: kd_cabang, cabang, kd_uker, uker, nama_agen, id_agen, kelas, no_telpon, bidang_usaha, Norek, casa</li>
            <li>File CSV menggunakan delimiter semicolon (;)</li>
        </ul>
    </div>

    <form action="{{ route('brilink.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
    ...
    @php
        $totalBrilink = \App\Models\Brilink::count();
    @endphp
    ...
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">
            Menghapus semua data Brilink akan menghapus <strong>{{ number_format($totalBrilink, 0, ',', '.') }} data</strong> secara permanen dan tidak dapat dikembalikan.
        </p>
        <form action="{{ route('brilink.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA data Brilink!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">
                🗑️ Hapus Semua Data Brilink
            </button>
        </form>
    </div>
    @endif
