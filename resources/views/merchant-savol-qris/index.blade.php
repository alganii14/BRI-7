...
<div class="header-actions">
    <div style="display: flex; gap: 10px;">
        ...
        <form action="{{ route('merchant-savol-qris.delete-all') }}" method="POST" style="display: inline;" onsubmit="return confirm('⚠️ PERHATIAN!\n\nAnda akan menghapus SEMUA data Merchant QRIS ({{ number_format($merchants->total(), 0, ',', '.') }} baris).\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda yakin ingin melanjutkan?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger-gradient">🗑️ Hapus Semua</button>
        </form>
        @endif
        <a href="{{ route('merchant-savol-qris.create') }}" class="btn btn-primary">➕ Tambah Data</a>
        <a href="{{ route('merchant-savol-qris.import.form') }}" class="btn btn-success">📁 Import CSV</a>
    </div>
    
    <form method="GET" action="{{ route('merchant-savol-qris.index') }}" class="search-form">
        <input type="text" name="search" placeholder="Cari merchant, CIF, store ID..." value="{{ request('search') }}">
        <button type="submit" class="btn btn-primary">🔍 Cari</button>
        @if(request('search'))
            <a href="{{ route('merchant-savol-qris.index') }}" class="btn btn-warning">✖ Reset</a>
        @endif
    </form>
</div>
...
@php
    $latestTanggalPosisi = \App\Models\MerchantSavolQris::whereNotNull('tanggal_posisi_data')
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->posisi_sv_total }}</td>
                <td>{{ $item->saldo_posisi }}</td>
                <td>
                    <div class="actions">
                        <a href="{{ route('merchant-savol-qris.edit', $item->id) }}" class="btn btn-sm btn-warning">✏️ Edit</a>
                        <form action="{{ route('merchant-savol-qris.destroy', $item->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
            ...
            <tr>
                <td colspan="13" style="text-align: center; padding: 40px;">
                    <p style="color: #999; font-size: 16px;">Tidak ada data Merchant QRIS.</p>
                    <a href="{{ route('merchant-savol-qris.import.form') }}" class="btn btn-success" style="margin-top: 10px;">Import CSV</a>
                </td>
            </tr>
            @endforelse
        </tbody>
...
<div class="pagination-wrapper" style="text-align: center; margin-top: 30px;">
    <p style="color: #666; font-size: 14px; margin: 0 0 15px 0;">Showing {{ $merchants->firstItem() }} to {{ $merchants->lastItem() }} of {{ $merchants->total() }} results</p>
    ...
    <div style="display: flex; justify-content: center; align-items: center; gap: 5px; flex-wrap: wrap;">
        ...
        @else
            <a href="{{ $merchants->appends(request()->query())->previousPageUrl() }}" style="padding: 8px 16px; background: white; color: #007bff; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; font-size: 14px; transition: all 0.2s;">← Previous</a>
        @endif
        ...
        @for ($page = $startPage; $page <= $endPage; $page++)
            ...
            @else
                <a href="{{ $merchants->appends(request()->query())->url($page) }}" style="padding: 8px 14px; background: white; color: #007bff; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; font-size: 14px; transition: all 0.2s; min-width: 40px; text-align: center;">{{ $page }}</a>
            @endif
        @endfor

        {{-- Next Button --}}
        @if ($merchants->hasMorePages())
            <a href="{{ $merchants->appends(request()->query())->nextPageUrl() }}" style="padding: 8px 16px; background: white; color: #007bff; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; font-size: 14px; transition: all 0.2s;">Next →</a>
        @else
