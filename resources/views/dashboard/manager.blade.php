...
<div class="card filter-card" style="margin-bottom: 24px; background: linear-gradient(135deg, #0066AE 0%, #004A7F 100%); border: none; box-shadow: 0 8px 24px rgba(0, 102, 174, 0.25);">
    <div class="filter-card-inner" style="background: white; padding: 24px; border-radius: 8px;">
        ...
        <form method="GET" action="{{ route('dashboard') }}">
            <div class="filter-grid" style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 1fr; gap: 16px; margin-bottom: 16px;">
            ...
            <div style="display: flex; gap: 12px;">
                ...
                @if($selectedRmftId || $selectedMonth != \Carbon\Carbon::now()->month || $selectedYear != \Carbon\Carbon::now()->year || $startDate || $endDate)
    ...
    <div style="padding: 20px;">
        @php
            ...
            foreach ($dateRange as $date) {
                $queryTarget = \App\Models\Aktivitas::where('nama_kc', Auth::user()->nama_kanca)
                    ->whereDate('tanggal', $date);
                $queryRealisasi = \App\Models\Aktivitas::where('nama_kc', Auth::user()->nama_kanca)
                    ->whereDate('tanggal', $date)
        ...
        <div style="position: relative; height: 350px; margin-top: 24px;">
            <canvas id="targetRealisasiChartManager"></canvas>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const ctx = document.getElementById('targetRealisasiChartManager').getContext('2d');
