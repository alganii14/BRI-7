    ...
    <div class="filter-card-inner" style="background: white; padding: 24px; border-radius: 8px;">
        ...
        <form method="GET" action="{{ route('dashboard') }}" id="filterForm">
            <div class="filter-grid" style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label for="rmft_id" style="display: block; margin-bottom: 8px; color: #004A7F; font-weight: 600; font-size: 14px;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="margin-right: 4px; vertical-align: text-top;">
                            <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/>
                        </svg>
                        RMFT
                    </label>
                    <select name="rmft_id" id="rmft_id" class="form-control" style="border: 2px solid #E5E7EB; border-radius: 8px; padding: 10px 12px; font-size: 15px; transition: all 0.3s ease; background: white;" onfocus="this.style.borderColor='#0066AE'; this.style.boxShadow='0 0 0 3px rgba(0, 102, 174, 0.1)'" onblur="this.style.borderColor='#E5E7EB'; this.style.boxShadow='none'">
                        <option value="">Semua RMFT</option>
                        @foreach($rmftList as $rmft)
                            <option value="{{ $rmft->id }}" {{ $selectedRmftId == $rmft->id ? 'selected' : '' }}>
                                {{ $rmft->completename }} - {{ $rmft->pernr }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div style="display: flex; gap: 12px;">
                ...
                @if($selectedKc || $selectedRmftId || $selectedMonth != \Carbon\Carbon::now()->month || $selectedYear != \Carbon\Carbon::now()->year || $startDate || $endDate)
...
<div class="dashboard-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 24px; margin-bottom: 24px;">
    <div class="card">
        ...
        <p style="font-size: 13px; color: #666; margin-top: 8px;">Total aktivitas {{ \Carbon\Carbon::now()->format('F') }}</p>
    </div>
    ...
    <div style="padding: 20px;">
        @php
            ...
            foreach ($dateRange as $date) {
                $queryTarget = \App\Models\Aktivitas::whereDate('tanggal', $date);
                $queryRealisasi = \App\Models\Aktivitas::whereDate('tanggal', $date)
                    ->whereIn('status_realisasi', ['tercapai', 'lebih']);
                
                if($selectedKc) {
                    $queryTarget->where('kode_kc', $selectedKc);
                    $queryRealisasi->where('kode_kc', $selectedKc);
                }
                
                if($selectedRmftId) {
                    $queryTarget->where('rmft_id', $selectedRmftId);
                    $queryRealisasi->where('rmft_id', $selectedRmftId);
                }
                
                $dailyData[] = [
                    'day' => $date->format('d/m'),
                    'target' => $queryTarget->sum('rp_jumlah'),
                    'realisasi' => $queryRealisasi->sum('nominal_realisasi')
                ];
            }
        ...
        <div style="position: relative; height: 350px; margin-top: 24px;">
            <canvas id="targetRealisasiChartAdmin"></canvas>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const ctx = document.getElementById('targetRealisasiChartAdmin').getContext('2d');
    const chartData = {!! json_encode($dailyData) !!};
    const labels = chartData.map(d => d.day);
    const targetData = chartData.map(d => d.target / 1000000);
    ...
    new Chart(ctx, {
        ...
        options: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        title: function(context) {
                            ...
                            @else
                                return 'Tanggal ' + context[0].label + ' {{ \Carbon\Carbon::create($selectedYear, $selectedMonth, 1)->translatedFormat("M Y") }}';
                            @endif
                        },
            ...
            scales: {
                x: {
                    title: { display: true, text: 'Tanggal', font: { size: 13, weight: 'bold' } },
                    grid: { display: false }
                },
                y: {
                    beginAtZero: true,
                    title: { display: true, text: 'Nominal (Juta Rupiah)', font: { size: 13, weight: 'bold' } },
                    ticks: { callback: function(value) { return 'Rp ' + value.toFixed(0) + ' juta'; } },
...
<script>
// Load RMFT ketika halaman pertama kali dimuat dan KC sudah terpilih
document.addEventListener('DOMContentLoaded', function() {
    const kcSelect = document.getElementById('kode_kc');
    const selectedKc = kcSelect.value;
    const selectedRmftId = '{{ $selectedRmftId }}';
    
    // Jika KC sudah terpilih (bukan "Semua KC"), load RMFT via AJAX
    if (selectedKc && selectedKc !== '') {
        loadRmftByKc(selectedRmftId);
    }
});

function loadRmftByKc(selectedRmftId = null) {
    const kcCode = document.getElementById('kode_kc').value;
    const rmftSelect = document.getElementById('rmft_id');
    
    // Simpan selected RMFT ID jika tidak diberikan sebagai parameter
    if (selectedRmftId === null) {
        selectedRmftId = rmftSelect.value;
    }
    
    // Reset dan disable dropdown RMFT
    rmftSelect.innerHTML = '<option value="">Loading...</option>';
    rmftSelect.disabled = true;
    
    if (!kcCode || kcCode === '') {
        // Jika KC kosong, reload halaman untuk menampilkan semua RMFT
        window.location.href = '{{ route("dashboard") }}?month=' + document.getElementById('month').value + '&year=' + document.getElementById('year').value;
        return;
    }
    
    // Load RMFT berdasarkan KC
    fetch('{{ route("api.rmft.by-kc") }}?kode_kc=' + encodeURIComponent(kcCode))
        .then(response => response.json())
        .then(data => {
            rmftSelect.innerHTML = '<option value="">Semua RMFT</option>';
            
            if (data.length === 0) {
                rmftSelect.innerHTML += '<option value="" disabled>Tidak ada RMFT di KC ini</option>';
            } else {
                data.forEach(rmft => {
                    const option = document.createElement('option');
                    option.value = rmft.id;
                    option.textContent = rmft.completename + ' - ' + rmft.pernr;
                    
                    // Set selected jika ID cocok
                    if (selectedRmftId && rmft.id == selectedRmftId) {
                        option.selected = true;
                    }
                    
                    rmftSelect.appendChild(option);
                });
            }
        ...
        .catch(error => {
            console.error('Error loading RMFT:', error);
            rmftSelect.innerHTML = '<option value="">Error loading data</option>';
            rmftSelect.disabled = false;
        });
}
</script>
