...
@section('title', 'Import CSV Penurunan Prioritas Ritel & Mikro')
@section('page-title', 'Import Data Penurunan Prioritas Ritel & Mikro dari CSV')
...
<div class="import-container">
    ...
    <div class="info-box">
        ...
        <ul>
            <li>File harus dalam format CSV (.csv) dengan delimiter semicolon (;)</li>
            <li>Pastikan format CSV sesuai dengan template yang ada</li>
            <li>Kolom CSV: Kode Cabang Induk, Cabang Induk, Kode Uker, Unit Kerja, CIFNO, No Rekening, Nama Nasabah, Segmentasi BPR, Jenis Simpanan, Saldo Last EOM, Saldo Terupdate, Delta</li>
            <li>Format angka menggunakan format Indonesia (. sebagai pemisah ribuan, , sebagai desimal)</li>
        </ul>
    </div>

    <form action="{{ route('penurunan-prioritas-ritel-mikro.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
    ...
    @php
        $totalData = \App\Models\PenurunanPrioritasRitelMikro::count();
    @endphp
    ...
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <form action="{{ route('penurunan-prioritas-ritel-mikro.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA {{ number_format($totalData, 0, ",", ".") }} data penurunan prioritas ritel & mikro!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
