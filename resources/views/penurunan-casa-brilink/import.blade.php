...
@section('title', 'Import CSV Penurunan Casa Brilink')
@section('page-title', 'Import Data Penurunan Casa Brilink dari CSV')
...
<div class="import-container">
    ...
    <div class="info-box">
        ...
        <ul>
            ...
            <li>Kolom CSV (dengan delimiter semicolon ;): Kode Cabang Induk, Cabang Induk, Kode Uker, Unit Kerja, CIFNO, No Rekening, Nama Nasabah, Jenis Simpanan, Saldo Last EOM, Saldo Terupdate, Delta</li>
        </ul>
    </div>

    <form action="{{ route('penurunan-casa-brilink.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('penurunan-casa-brilink.index') }}" class="btn btn-secondary">
                ↩️ Kembali
            </a>
        </div>
    </form>
    ...
    @php
        $totalPenurunanCasaBrilink = \App\Models\PenurunanCasaBrilink::count();
    @endphp

    @if($totalPenurunanCasaBrilink > 0)
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">
            Menghapus semua data penurunan casa brilink akan menghapus <strong>{{ number_format($totalPenurunanCasaBrilink, 0, ',', '.') }} data</strong> secara permanen dan tidak dapat dikembalikan.
        </p>
        <form action="{{ route('penurunan-casa-brilink.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA {{ number_format($totalPenurunanCasaBrilink, 0, ",", ".") }} data penurunan casa brilink!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">
                🗑️ Hapus Semua Data Penurunan Casa Brilink
            </button>
        </form>
    </div>
    @endif
