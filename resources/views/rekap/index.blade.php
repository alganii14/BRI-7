...
<div class="card">
    ...
    <div class="pagination-wrapper">
        <p style="color: #666; font-size: 14px; margin: 0 0 15px 0;">Showing {{ $rekaps->firstItem() }} to {{ $rekaps->lastItem() }} of {{ $rekaps->total() }} results</p>
        ...
        <div style="display: flex; justify-content: center; align-items: center; gap: 5px; flex-wrap: wrap;">
            ...
            @else
                <a href="{{ $rekaps->previousPageUrl() }}" style="padding: 8px 16px; background: white; color: #007bff; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; font-size: 14px; transition: all 0.2s;">← Previous</a>
            @endif
            ...
            @for ($page = $startPage; $page <= $endPage; $page++)
                ...
                @else
                    <a href="{{ $rekaps->url($page) }}" style="padding: 8px 14px; background: white; color: #007bff; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; font-size: 14px; transition: all 0.2s; min-width: 40px; text-align: center;">{{ $page }}</a>
                @endif
            @endfor

            {{-- Next Button --}}
            @if ($rekaps->hasMorePages())
                <a href="{{ $rekaps->nextPageUrl() }}" style="padding: 8px 16px; background: white; color: #007bff; border: 1px solid #dee2e6; border-radius: 4px; text-decoration: none; font-size: 14px; transition: all 0.2s;">Next →</a>
            @else
