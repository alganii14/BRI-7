...
@section('page-title', 'Data Layering')
...
<div class="header-actions">
    <div style="display: flex; gap: 10px;">
        @if($layerings->total() > 0)
        <form action="{{ route('layering.delete-all') }}" method="POST" style="display: inline;" onsubmit="return confirm('⚠️ PERHATIAN!\n\nAnda akan menghapus SEMUA data layering ({{ number_format($layerings->total(), 0, ',', '.') }} baris).\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda yakin ingin melanjutkan?')">
            @csrf
            @method('DELETE')
        ...
        @endif
        <a href="{{ route('layering.create') }}" class="btn btn-primary">
            ➕ Tambah Data
        </a>
        <a href="{{ route('layering.import.form') }}" class="btn btn-success">
            📁 Import CSV
        </a>
    </div>
    
    <form method="GET" action="{{ route('layering.index') }}" class="search-form" style="display:flex;gap:10px;align-items:end;">
        <div style="flex:1;">
        ...
        @if(request('search') || request('month') || request('year'))
            <a href="{{ route('layering.index') }}" class="btn btn-warning">✖ Reset</a>
        @endif
    </form>
</div>
...
@php
    $latestTanggalPosisi = \App\Models\Layering::whereNotNull('tanggal_posisi_data')
...
<div class="table-container">
    <table>
        ...
        <tbody>
            @forelse($layerings as $index => $item)
            <tr>
                <td>{{ $layerings->firstItem() + $index }}</td>
                ...
                <td>
                    <div class="actions">
                        <a href="{{ route('layering.show', $item->id) }}" class="btn btn-sm btn-info">👁️ View</a>
                        <a href="{{ route('layering.edit', $item->id) }}" class="btn btn-sm btn-warning">✏️ Edit</a>
                        <form action="{{ route('layering.destroy', $item->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
            ...
            <tr>
                <td colspan="14" style="text-align: center; padding: 40px;">
                    <p style="color: #999; font-size: 16px;">Tidak ada data layering.</p>
                    <a href="{{ route('layering.import.form') }}" class="btn btn-success" style="margin-top: 10px;">Import CSV</a>
                </td>
            </tr>
            @endforelse
        </tbody>
...
<div class="pagination-wrapper">
    <p class="pagination-info">Showing {{ $layerings->firstItem() }} to {{ $layerings->lastItem() }} of {{ $layerings->total() }} results</p>
    
    <div style="display: flex; justify-content: center; gap: 10px; margin-top: 15px; flex-wrap: wrap;">
        @if ($layerings->onFirstPage())
        ...
        @else
            <a href="{{ $layerings->previousPageUrl() }}" style="padding: 10px 20px; background: white; color: #333; border: 1px solid #ddd; border-radius: 4px; cursor: pointer; text-decoration: none;">← Previous</a>
        @endif

        @php
            $currentPage = $layerings->currentPage();
            $lastPage = $layerings->lastPage();
            $startPage = 1;
            $endPage = min(5, $lastPage);
        @endphp

        @foreach (range($startPage, $endPage) as $page)
            @php $url = $layerings->url($page); @endphp
            @if ($page == $currentPage)
        ...
        @if ($layerings->hasMorePages())
            <a href="{{ $layerings->nextPageUrl() }}" style="padding: 10px 20px; background: white; color: #333; border: 1px solid #ddd; border-radius: 4px; cursor: pointer; text-decoration: none;">Next →</a>
        @else
