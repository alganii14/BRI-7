...
@section('title', 'Import CSV AUM DPK')
@section('page-title', 'Import Data AUM>2M DPK<50 juta dari CSV')
...
<div class="import-container">
    ...
    <form action="{{ route('aum-dpk.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
        @csrf
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('aum-dpk.index') }}" class="btn btn-secondary">
                ↩️ Kembali
            </a>
        </div>
    </form>
    ...
    @php
        $totalAumDpk = \App\Models\AumDpk::count();
    @endphp
    ...
    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 16px;">
            Menghapus semua data AUM DPK akan menghapus <strong>{{ number_format($totalAumDpk, 0, ',', '.') }} data</strong> secara permanen dan tidak dapat dikembalikan.
        </p>
        <form action="{{ route('aum-dpk.delete-all') }}" method="POST" onsubmit="return confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA {{ number_format($totalAumDpk, 0, ",", ".") }} data AUM DPK!\n\nData yang sudah dihapus TIDAK DAPAT dikembalikan!\n\nApakah Anda BENAR-BENAR yakin?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">
                🗑️ Hapus Semua Data AUM DPK
            </button>
        </form>
    </div>
    @endif
